...
def train_test(train_model_spec, test_model_spec, params, vocab_rev, restore_from=None):
    ...
    with tf.Session() as sess:
        ...
        for epoch_id in range(begin_at_epoch, begin_at_epoch + params.num_epochs):
            ...
            batch_loss = train_sess(sess, train_model_spec, train_num_steps, params)
...
def test_sess(text, model_spec, params, vocab_rev, sess=None, restore_from=None):
    ...
    text_tf = model_spec['text_tf']
    predictions = model_spec['inference']
    predictions = tf.cast(predictions, tf.int64)
    predicted_chars = vocab_rev.lookup(predictions)
    to_return = {'predictions': predicted_chars}
    results = curr_sess.run(to_return, feed_dict={text_tf: [text]})
    if not sess: curr_sess.close()
    preds = results['predictions']
    preds_string = [''.join([c.decode('UTF-8') for c in chars]) for chars in preds]
    return preds_string
    #return results['chars'], results['probs']
