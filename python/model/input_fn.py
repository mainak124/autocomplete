...
def test_input_fn(mode, vocab, params):
    text_tf_till_t = tf.placeholder(tf.string, shape=[None], name='text_till_t')
    text_tf_at_t = tf.placeholder(tf.string, shape=[None], name='text_at_t')
    src_sequence_length = tf.placeholder(tf.int32, shape=[None], name='src_sequence_length')
